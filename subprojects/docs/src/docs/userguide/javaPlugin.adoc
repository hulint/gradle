...
The main goals for incremental compilations are:
...
* A class is recompiled if its source has been changed or if one of its transitive dependencies has been changed.
* A classes dependencies are determined from type references in its bytecode.
* Since constants can be inlined, any change to a constant will result in full recompilation. Try to minimize use of constants and replace them with static methods where possible.
...
* The class analysis is cached in the project directory, so a clean checkout can be slower. Consider turning off the incremental compiler on CI.
...
Please first have a look at the chapter about incremental compilation above, as incremental annotation processing builds on top of it.
...
* Must not have any side effects except for using the `Filer` and `Messager` APIs.
...
* Must not depend on compiler-specific APIs like `com.sun.source.util.Trees`. Gradle wraps the processing APIs, so attempts to cast to compiler-specific types will fail.
If your processor does this, it cannot be incremental, unless you have some fallback mechanism.
* Using `Filer#getResource` currently results in full recompilation. See https://github.com/gradle/gradle/issues/4701[gradle/issues/4701]
* Using `Filer#createResource` currently results in full recompilation. See https://github.com/gradle/gradle/issues/4702[gradle/issues/4702]
...
* Must make all decisions (code generation, validation messages) for an annotated type based on information reachable from its AST.
...
* Must provide exactly one originating element for each file generated with the `Filer` API. Providing any other number of elements will result in full recompilation.


When a source file is recompiled, Gradle will recompile all files generated from it.
If a source file is deleted, the files generated from it are deleted.
...
* Can only read `CLASS` or `RUNTIME` retention annotations
* Can only read parameter names if the user passes the `-parameters` compiler argument.
